...
class SimpleTerminalApp:
    def __init__(self, root):
        ...
        self.zip_file_path = "filesys.zip"
        # Text box for output
            ...
            if command == 'clear':
                ...
                if len(parsed_command) == 1:
                    ...
                    if self.get_absolute_path(parsed_command[1])[-1] == '/':
                        ...
                        path = self.get_absolute_path(parsed_command[1]) + '/'
                with zipfile.ZipFile(self.zip_file_path) as zip_file:
                    files = set()
                    dirs = set()
                ...
                temp_zip_path = self.zip_file_path + '.tmp'
                with zipfile.ZipFile(self.zip_file_path, 'r') as zip_to_read:
                ...
                os.replace(temp_zip_path, self.zip_file_path)
            elif command == 'rm':
                temp_zip_path = self.zip_file_path + '.tmp'
                with zipfile.ZipFile(self.zip_file_path, 'r') as zip_to_read:
                    with zipfile.ZipFile(temp_zip_path, 'w') as zip_to_write:
                        for item in zip_to_read.infolist():
                            if not (item.filename == parsed_command[1]):
                                zip_to_write.writestr(item, zip_to_read.read(item.filename))
                os.replace(temp_zip_path, self.zip_file_path)
            elif command == 'uptime':
                ...
                zip_temp_path = self.zip_file_path + '.tmp'
                ...
                with zipfile.ZipFile(self.zip_file_path, 'r') as zip_to_read:
                    with zipfile.ZipFile(zip_temp_path, 'w') as zip_to_write:
                        for item in zip_to_read.infolist():
                            ...
                            zip_to_write.writestr(item, zip_to_read.read(item.filename))
                os.replace(zip_temp_path, self.zip_file_path)
            elif command == "cd":
                # не работает ../
                abs_path = self.get_absolute_path(parsed_command[1])
                ...
                output = subprocess.check_output(unparsed_command, cwd=self.zip_file_path, stderr=subprocess.STDOUT,
